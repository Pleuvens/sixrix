...
int main(int argc, char* argv[]) {
	(void)argc;
	printf("Extracting signal from audio file... ");
	double *signal = signalArray(argv[1]);
	frameNbr_ = frameNbr();
	long frameSampleNbr_ = frameSampleNbr();
	printf("Done:\n	Size of the audio: %d bytes\n	Sampling rate: %d Hertz\n	Number of samples: %ld samples\n	Number of samples in one frame of 20 millisecond (standard): %ld\n	Number of 20 ms frames for this audio file (overlap of 10 ms): %ld\n",header.overall_size,	header.sample_rate,	num_samples,	frameSampleNbr_,	frameNbr_);
	/*

double **MFCC(char *file) {
	double *signal = signalArray(file);
*/
	printf("\nPre-emphasis of the signal... Done.\n");
	double *signal_pre_emphasis = pre_emphasis(signal);
	//free(signal);
	printf("\nFraming the signal and applying Hanning Window on each... Done.\n");
	double **framed_signal = hannWindow(signal_pre_emphasis);
	free(signal_pre_emphasis);
	printf("\nApplying the Fast Fourier Transform on each frame... Done.\n");
	cplx **DFTed_frames = DFT(framed_signal);
	/*for (long i = 0; i < frameNbr_; i++) {
	  free(framed_signal[i]);
	  }
	  free(framed_signal);*/
	printf("\nComputing the power spectrum of the framed signal... Done.\n");
	double **power_spectrum = PEPS(DFTed_frames);
	/*for (long i = 0; i < frameNbr_; i++) {
		free(DFTed_frames[i]);
	}
	free(DFTed_frames);*/
	printf("\nComputing the Mel Filterbank (26 filters)... Done.\n");
	double **filterBank = filterbank(sampleRate);
	printf("\nComputing the energies of each frame and computing their log... Done.\n");
	...
	printf("\nComputing the Discrete Cosine Transform on these energie logs... Done.\n");
	 ...
	 */
	printf("\nExamples of the first feature vectors :");
