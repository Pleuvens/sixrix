...
//  int rendu;
//  if((rendu = open("data_phonemes", O_WRONLY/|O_TRUNC|O_APPEND, 0666)) == -1)
//    err(3, "fail");
  FILE *rendu = fopen("data_phonemes", "r+w");
  fseek(rendu, 0, SEEK_END);
  ...
  if (size_is_correct) { 				
    ...
    //  int w;
      ...
      relou[0] = testy[i];
      relou[1] = '\0';
      fprintf(rendu, "%s", (const char*)relou);
  // if((w = write(rendu, (const char*)relou, sizeof(char))) == -1)
//	errx(3, "fail");
    }
  }
  fprintf(rendu, "û");
